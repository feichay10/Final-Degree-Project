...
confirm_action() {
    ...
    echo "   • Las imágenes: dhcp_server, dns_primary_server, dns_secondary_server"
    echo -e "${NC}"
...
cleanup_docker() {
    ...
    images=("dhcp_server:latest" "dns_primary_server:latest" "dns_secondary_server:latest")
...
show_summary() {
    ...
    echo -e "${WHITE}Imágenes del proyecto:${NC}"
    project_images=$(docker images --format "table {{.Repository}}\t{{.Tag}}\t{{.Size}}" | grep -E "(dhcp_server|dns_primary_server|dns_secondary_server)" || echo "")
    if [ -z "$project_images" ]; then
        echo -e "${GREEN}   ✅ Todas las imágenes del proyecto han sido eliminadas${NC}"
    else
        echo "$project_images"
    fi
}
...
main() {
    ...
    echo ""
    echo -e "${GREEN}🚀 ¡Listo para reconstruir los servicios!${NC}"
